...
public class TeamDAOImpl implements TeamDAO {
	...
	public List<Team> getAllTeams() throws BusinessException {
		...
		try (Connection connection = PostgresConnection.getConnection()){
			String sql = "select teamid,teamname,location from player_schema.team order by teamid";
			...
			if(teamList.size() == 0) {
				throw new BusinessException("No teams registered so far...Please use the Add Team option to add a team...");
			}
		} catch(ClassNotFoundException | SQLException e) {
			System.out.println(e);
			throw new BusinessException("Internal error has occured...Please contact SYS ADMIN...");
		}
		return teamList;
	}
